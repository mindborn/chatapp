...
async function registerclientpost(req, res) {
    ...
    mongo.MongoDBQuery('usercurrent', { Client_ID: Client_ID, password: password }, (docs) => {
        ...
        if (docs.length > 0) {
           ...
           /* mongo.MongoDBOperation(function (database) {
                database.collection('userlogin').insertOne(userlogin);
            });*/
            mongo.MongoDBQuery('usercurrent', {Client_ID:Client_ID }, (docs) => {
                if (docs.length > 0) {
                    ...
                    var userlogin =
                    {
                        ...
                        Client_ID:Client_ID,
                       token:token,
                       Name:docs[0].firstname
                    };
                    mongo.MongoDBOperation(function (database) {
                        database.collection('usernewlogin').insertOne(userlogin);
                    })
                }
            }); 
            res.send(JSON.stringify(result));
